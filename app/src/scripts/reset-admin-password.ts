/**
 * Rotates the admin password to a strong random value and prints it once.
...
async function resetAdminPassword() {
    ...
    try {
        const rows: any = await query("SELECT id FROM tenants WHERE email = ? AND role = 'admin'", [email]);
