...
(function () {
    ...
    document.addEventListener('DOMContentLoaded', function () {
        ...
        var modalContent = modal ? modal.querySelector('.cataseek-modal-content') : null;
        var modalOverlay = modal ? modal.querySelector('.cataseek-modal-overlay') : null;
        var modalBody = modal ? modal.querySelector('.cataseek-modal-body') : null;
        var contentArea = modal ? modal.querySelector('.cataseek-content-area') : null;
        ...
        var sortingContainer = modal ? modal.querySelector('.cataseek-sorting') : null;
        var voiceBtn = document.getElementById('cataseek-voice-search-btn');
        // --- Suggestion System DOM Refs ---
        var searchInputWrapper = modal ? modal.querySelector('.cataseek-search-input-wrapper') : null;
        ...
        if (!modal) return;
        // Apply theme color
        document.documentElement.style.setProperty('--cataseek-color', config.color);
        document.documentElement.style.setProperty('--cataseek-color-dark', shadeColor(config.color, -20));
            ...
            if (selectorTargets.length > 0) {
                ...
                selectorTargets.forEach(function (target) {
                    ...
                    target.addEventListener('click', function (e) {
                        ...
                        e.stopPropagation();
                        openModal();
                    }, true); // use capture so we intercept before PS handlers
                    ...
                    target.querySelectorAll('input[type="text"], input[type="search"], textarea').forEach(function (inp) {
                ...
                setTimeout(function () {
                    try {
                        document.querySelectorAll(customSelector).forEach(function (target) {
                            target.addEventListener('click', function (e) {
                                e.preventDefault();
                                e.stopPropagation();
                                openModal();
                            }, true);
                            target.querySelectorAll('input[type="text"], input[type="search"]').forEach(function (inp) {
                                inp.addEventListener('focus', function () { inp.blur(); openModal(); });
                                inp.addEventListener('click', function (e) { e.preventDefault(); e.stopPropagation(); openModal(); }, true);
                            });
                            var form = (target.tagName === 'FORM') ? target : (target.closest('form') || target.querySelector('form'));
                            if (form) form.addEventListener('submit', function (e) { e.preventDefault(); openModal(); });
                        });
                        if (triggerBtn) triggerBtn.style.display = 'none';
        ...
        if (mobileFilterBtn) {
            mobileFilterBtn.addEventListener('click', function (e) {
                e.preventDefault();
                if (filtersSidebar) filtersSidebar.classList.add('cataseek-filters-open');
        ...
        document.addEventListener('keydown', function (e) {
            if (e.key === 'Escape' && modal.classList.contains('cataseek-modal-active')) {
                closeModal();
            }
        });
        if (searchInput) {
        ...
        if (SpeechRecognition && voiceBtn) {
            ...
            var langFallbackMap = {
                ...
                'uk': 'uk-UA', 'el': 'el-GR', 'he': 'he-IL', 'hi': 'hi-IN',
                'th': 'th-TH', 'vi': 'vi-VN', 'id': 'id-ID', 'ms': 'ms-MY'
            };
            if (voiceLang && !voiceLang.includes('-') && !voiceLang.includes('_')) {
                voiceLang = langFallbackMap[voiceLang.toLowerCase()] || (voiceLang + '-' + voiceLang.toUpperCase());
            }
            ...
            var onVoiceStart = function () {
                ...
                if (searchInput) {
                    searchInput.placeholder = voiceLang.toLowerCase().indexOf('es') === 0 ? 'Escuchando...' : 'Listening...';
                }
            };
            ...
            var onVoiceError = function (event) {
                ...
                if (event.error === 'not-allowed' || event.error === 'service-not-allowed') {
                    ...
                    if (searchInput) {
                        searchInput.placeholder = searchInput.getAttribute('placeholder') ||
                            (voiceLang.toLowerCase().indexOf('es') === 0 ? 'Buscar productos...' : 'Search for products...');
                    }
                }
            };
        ...
        function closeMobileFilters() {
            if (filtersSidebar) filtersSidebar.classList.remove('cataseek-filters-open');
            if (filterOverlay) filterOverlay.classList.remove('cataseek-filters-open');
        }
        // --- Search Logic ---
            ...
            fetch(apiUrl + '/products/public/search', {
                ...
                .then(function (data) {
                    ...
                    if (!isLoadMore) {
                        ...
                        if (state.facetStats && state.facetStats.price && state.priceRange.min === null) {
                    ...
                    state.hasMore = (state.offset + (data.results ? data.results.length : 0)) < state.total;
                })
                .catch(function (err) {
        ...
        function renderResults(hits) {
            ...
            var html = '<div class="cataseek-products-grid">' +
                hits.map(function (h) { return productCard(h, state.query); }).join('') +
                '</div>';
        ...
        function appendResults(hits) {
            ...
            temp.innerHTML = hits.map(function (h) { return productCard(h, state.query); }).join('');
        ...
        function productCard(product, query) {
            ...
            var cartSvg = '<svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">' +
                '<path d="M6 2 3 6v14a2 2 0 0 0 2 2h14a2 2 0 0 0 2-2V6l-3-4Z"></path>' +
                '<path d="M3 6h18"></path>' +
                '<path d="M16 10a4 4 0 0 1-8 0"></path>' +
                '</svg>';
            return '<a href="' + url + '" class="cataseek-product-link">' +
                '<div class="cataseek-product-item">' +
                '<div class="cataseek-product-image">' +
                (imgSrc ? '<img src="' + escapeHtml(imgSrc) + '" alt="" loading="lazy">' : '') +
                '</div>' +
                '<div class="cataseek-product-info">' +
                '<h4 class="cataseek-product-name">' + name + '</h4>' +
                '<div class="cataseek-product-price">' +
                '<span class="cataseek-price-current">' + price + '</span>' +
                '<button class="cataseek-add-to-cart" data-pid="' + pid + '" title="Add to cart">' + cartSvg + '</button>' +
                '</div>' +
                '</div>' +
                '</div>' +
                '</a>';
        ...
        function expandToQuantityPicker(cartBtn) {
            ...
            wrapper.innerHTML =
                ...
                '<button class="cataseek-qty-confirm" data-pid="' + pid + '">Add</button>';
        ...
        function addToCart(confirmBtn) {
            ...
            var qty = parseInt(wrapper.querySelector('.cataseek-qty-input').value) || 1;
            var token = (typeof prestashop !== 'undefined' && prestashop.static_token) ? prestashop.static_token : '';
            confirmBtn.textContent = '...';
            confirmBtn.disabled = true;
            fetch('/index.php?controller=cart', {
                method: 'POST',
                headers: { 'Content-Type': 'application/x-www-form-urlencoded' },
                body: 'action=update&add=1&ajax=1&qty=' + qty + '&id_product=' + encodeURIComponent(pid) + '&token=' + encodeURIComponent(token)
            })
            .then(function (r) {
                var status = r.status;
            ...
            .then(function (response) {
                ...
                if (data) {
                    if (data.success === true || data.hasError === false) {
                        isSuccess = true;
                    } else if (typeof data.success === 'undefined' && typeof data.hasError === 'undefined' && !data.errors) {
                ...
                if (isSuccess) {
                    ...
                    newBtn.className = 'cataseek-add-to-cart cataseek-add-to-cart--done';
                    newBtn.dataset.pid = pid;
                    newBtn.title = 'Added!';
                    newBtn.innerHTML = cartSvg;
                    wrapper.parentNode.replaceChild(newBtn, wrapper);
                } else {
                    console.error('[Cataseek] Server rejected cart add:', response.text);
                    showCartToast('error');
        ...
        function showCartToast(type) {
            ...
            toast.textContent = type === 'success' ? '\u2713 Added to cart!' : '\u2717 Could not add to cart';
        ...
        function updateCartCount(count) {
            ...
            if (count !== null && count !== undefined) {
                var counters = document.querySelectorAll('.cart-products-count, .header-cart-count, [data-cart-count]');
        ...
        function renderFilters() {
            ...
            if (state.priceRange.currentMin !== null || state.priceRange.currentMax !== null) {
                if (state.priceRange.currentMin !== state.priceRange.min || state.priceRange.currentMax !== state.priceRange.max) {
                    activeCount++;
                }
            }
            // Mobile Close Button
            html += '<button id="cataseek-filter-close-btn" class="cataseek-mobile-filter-close" aria-label="Close Filters">' +
                    '<svg width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">' +
                    '<line x1="18" y1="6" x2="6" y2="18"></line><line x1="6" y1="6" x2="18" y2="18"></line></svg>' +
                    '</button>';
            if (activeCount > 0) {
                html += '<div class="cataseek-filters-header-actions">';
                html += '<button id="cataseek-clear-filters-btn" class="cataseek-clear-filters-btn">' + (config.language === 'es' ? 'Limpiar filtros' : 'Clear all filters') + '</button>';
                html += '</div>';
            }
            // Categories
            if (state.facets && state.facets.categories) {
                ...
                if (keys.length > 0) {
                    hasFilters = true;
                    html += '<div class="cataseek-filter-group">' +
                        '<div class="cataseek-filter-title">Categorias</div>' +
                        '<ul class="cataseek-filter-list">' +
                        keys.map(function (cat) {
                            var checked = state.selectedFilters.categories.indexOf(cat) > -1;
                            return '<li class="cataseek-filter-item">' +
                                '<label class="cataseek-filter-label">' +
                                '<input type="checkbox" class="cataseek-filter-checkbox" value="' + escapeHtml(cat) + '" ' + (checked ? 'checked' : '') + ' data-type="categories">' +
                                '<span>' + escapeHtml(cat) + '</span>' +
                                '<span class="cataseek-filter-count">' + categories[cat] + '</span>' +
                                '</label>' +
                                '</li>';
                        }).join('') +
                        '</ul>' +
            ...
            // Price Slider
            if (state.priceRange.min !== null && state.priceRange.max !== null && state.priceRange.min < state.priceRange.max) {
                hasFilters = true;
                var curMin = state.priceRange.currentMin !== null ? state.priceRange.currentMin : state.priceRange.min;
                var curMax = state.priceRange.currentMax !== null ? state.priceRange.currentMax : state.priceRange.max;
                html += '<div class="cataseek-filter-group">' +
                    '<div class="cataseek-filter-title">Precio</div>' +
                    '<div class="cataseek-price-slider-container">' +
                    '<div class="cataseek-price-range-inputs">' +
                    '<span class="cataseek-price-input-box">' + Math.floor(curMin) + currency + '</span>' +
                    '<span class="cataseek-price-input-box">' + Math.ceil(curMax) + currency + '</span>' +
                    '</div>' +
                    ...
                    '<span>' + state.priceRange.min + currency + '</span>' +
                    '<span>' + Math.round((state.priceRange.min + state.priceRange.max) / 2) + currency + '</span>' +
                    '<span>' + state.priceRange.max + currency + '</span>' +
                    '</div>' +
                    '</div>' +
                    '</div>';
            }
            ...
            filtersSidebar.querySelectorAll('.cataseek-filter-checkbox').forEach(function (cb) {
                cb.addEventListener('change', function () {
                    var type = this.getAttribute('data-type');
                    var value = this.value;
                    if (this.checked) state.selectedFilters[type].push(value);
                    else state.selectedFilters[type] = state.selectedFilters[type].filter(function (v) { return v !== value; });
                    state.offset = 0;
                    performSearch();
                });
            });
        ...
        function initPriceSlider() {
            ...
            var curMin = state.priceRange.currentMin !== null ? state.priceRange.currentMin : minVal;
            var curMax = state.priceRange.currentMax !== null ? state.priceRange.currentMax : maxVal;
            function updateUI() {
            ...
            function onDrag(e, type) {
                ...
                var boxes = filtersSidebar.querySelectorAll('.cataseek-price-input-box');
                if (boxes.length === 2) {
            ...
            function startDrag(e, type) {
                e.preventDefault();
                var moveHandler = function (me) { onDrag(me, type); };
        ...
        function showDiscoveryState() {
            ...
            if (cachedDiscoveryHtml) {
                resultsContainer.innerHTML = '<div class="cataseek-discovery-section"><h3 class="cataseek-discovery-title">' + escapeHtml(trendingTitle) + '</h3>' + cachedDiscoveryHtml + '</div>';
                resultsContainer.style.display = 'block';
        ...
        function fetchDiscoveryProducts() {
            ...
            .then(function (data) {
                if (data.results && data.results.length > 0) {
                    cachedDiscoveryHtml = '<div class="cataseek-products-grid">' +
                        data.results.map(function(h) { return productCard(h, ''); }).join('') +
                        '</div>';
                } else {
                    cachedDiscoveryHtml = '<div class="cataseek-empty-state"><p>Start typing to search products...</p></div>';
                }
        ...
        function showEmptyState() {
            hideAllStates();
            resultsCountEl.innerText = '';
            resultsContainer.innerHTML = '<div class="cataseek-empty-state"><p>Start typing to search products...</p></div>';
            resultsContainer.style.display = 'block';
        }
        function showError(msg) {
            hideAllStates();
            resultsContainer.innerHTML = '<div class="cataseek-error-state"><p>Error: ' + escapeHtml(msg) + '</p></div>';
            resultsContainer.style.display = 'block';
        }
        ...
        function saveRecentSearch(q) {
            ...
            list = list.filter(function (s) { return s.toLowerCase() !== q.toLowerCase(); });
        ...
        function removeRecentSearch(q) {
            var list = loadRecentSearches().filter(function (s) { return s !== q; });
            try { localStorage.setItem(RECENT_KEY, JSON.stringify(list)); } catch (e) { }
        }
        ...
        function getResponsivePillCount() {
            var w = searchInputWrapper ? searchInputWrapper.offsetWidth : window.innerWidth;
            if (w < 320) return 2;
            if (w < 480) return 3;
            if (w < 640) return 4;
        ...
        function applyResponsivePillCount() {
            ...
            suggestionPills.classList.remove('cataseek-pills-2', 'cataseek-pills-3', 'cataseek-pills-4');
            if (count < 6) {
        ...
        function renderRecentPills() {
            ...
            if (!recents.length || !suggestionPills) { hideSuggestions(); return; }
            applyResponsivePillCount();
            var html = recents.map(function (q) {
                var escaped = escapeHtml(q);
                return '<button type="button" class="cataseek-suggestion-pill" data-query="' + escaped + '">' +
                    '<span class="cataseek-pill-bold">' + escaped + '</span>' +
                    '<button type="button" class="cataseek-pill-delete" data-delete="' + escaped + '" aria-label="Remove">&#x2715;</button>' +
                    '</button>';
            }).join('');
        ...
        function renderSuggestionPills(suggestions, inputValue) {
            if (!suggestions || !suggestions.length || !suggestionPills) { hideSuggestions(); return; }
            applyResponsivePillCount();

            var inputLower = inputValue.toLowerCase();

            var html = suggestions.map(function (sugg) {
                ...
                return '<button type="button" class="cataseek-suggestion-pill" data-query="' + escapeHtml(sugg) + '">' +
                    '<span class="cataseek-pill-bold">' + boldPart + '</span>' +
                    (lightPart ? '<span class="cataseek-pill-light">' + lightPart + '</span>' : '') +
                    '</button>';
            }).join('');
